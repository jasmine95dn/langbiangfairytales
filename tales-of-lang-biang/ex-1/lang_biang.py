...
class LSTMCharRNN():
    def __init__(self, 
        ...
        # link of Multilayer-LSTM cell https://www.tensorflow.org/api_docs/python/tf/nn/rnn_cell/LSTMCell
	...
	if argv[1] == 'training':
		...
		for i in range(restore+print_save_freq, restore+iterations+print_save_freq):
			...
			if i%print_save_freq==0:
			    ...
			    # save the model
			    save_path = saver.save(sess, "{}/model-{}".format(model_dir, i))
			    print("Model saved in file: {}".format(save_path))

	else:
		# create the model
		m = LSTMCharRNN(sequence_length=generation_seq_len, vocab_size=vocab_size, emb_size=emb_size, num_units=num_units, is_training=False, sampling=True, temperature=temperature, 
		      start_id=char2id["\n"])
		...
		saver.restore(sess, "{}/model-{}".format(load_model_dir, load_iteration))
		print("Model restored from iteration {}.\n".format(load_iteration))
		...
		for j in range(no_samples):
			...
			with open('lang_biang_fake.txt', 'a') as text:
